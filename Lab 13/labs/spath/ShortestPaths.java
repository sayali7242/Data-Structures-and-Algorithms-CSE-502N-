    ...
    public void run() {
    	...
    	if(pq.isEmpty()) {
	    	...
	    	while(!pq.isEmpty()){
	    		...
	    		for (Edge e : x.vertex.edgesFrom()){
	    			int oldDistance = weights.get(e) + x.distance;
	    			Decreaser<VertexAndDist> dec = handles.get(e.to);
	    			int newDistance = dec.getValue().distance;
	    			if(oldDistance < newDistance) {
	    				dec.decrease(new VertexAndDist(dec.getValue().vertex, oldDistance));
	    				parentEdges.put(dec.getValue().vertex, e);
	    			}
	    		}
	    	}
    	}
